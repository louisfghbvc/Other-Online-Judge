...
public:
    ...
    int lower_bound(int id, int l, int r, int x, int pos){
        if(l == r) return tre[id].mx >= x ? l : -1;
        int mid = (l+r)/2;
        int res = -1;
        if(tre[id*2].mx >= x && mid >= pos)
            res = lower_bound(id*2, l, mid, x, pos);
        if(res != -1)  return res;
        return lower_bound(id*2+1, mid+1, r, x, pos);
    }
};
...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){ // change index x to opposite
            ...
            int x, l; cin >> x >> l;
            cout << st.lower_bound(1, 0, n-1, x, l) << "\n";
        }
    }
    return 0;
}
